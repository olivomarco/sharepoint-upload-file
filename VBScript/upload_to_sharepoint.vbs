' GO TO LINE *257* FOR THE BEGINNING OF THE ACTUAL CODE

' ----------------------------------------------------------------------------------------------
' JSON parser implemented in VBScript - EDITED by Luca Giuliani to fix an integer overflow issue 
' original version from: https://github.com/gerritvankuipers/aspjson/blob/master/aspJSON1.19.asp
Class aspJSON
	Public data
	...
	Private Function aj_ReadNumericValue(ByVal val)
		...
		Else
			...
			If Err<>0 Then aj_ReadNumericValue=val
			On Error Goto 0
		End If
	End Function
End Class
...
M365_TENANT_NAME="<SHAREPOINT-HOSTNAME>"
SHP_SITE_NAME="<SHAREPOINT-SITE-NAME>"
...
postdata = "grant_type=client_credentials&client_id=" & client_id & "&client_secret=" & client_secret & "&scope=https://graph.microsoft.com/.default"
http.send postdata
...
http.send
oJSON.loadJSON http.responseText
drive_id = oJSON.data("id")

' upload a binary test file on our sharepoint drive and get its url
local_filename = "<FULL-PATH-TO-LOCAL-FILE-TO-UPLOAD>"
sharepoint_filename = "<FILENAME-ONLY-AS-IN-SHAREPOINT-FOLDER>"

Set ads = CreateObject("ADODB.Stream")
ads.Open
ads.LoadFromFile local_filename

request_url = "https://graph.microsoft.com/v1.0/drives/" + drive_id + "/items/root:/" + sharepoint_filename + ":/content"
http.open "PUT", request_url, False
http.setRequestHeader "Authorization", "Bearer " + access_token
